...
public class SignUpVC: UIViewController {
    ...
    var _vm: SignUpViewModel!
    public override func viewDidLoad() {
        super.viewDidLoad()
        _vm = SignUpViewModel(input: SignUpViewModel.Input(
        ...
        _vm.output.signInResult.subscribe(onNext: { [weak self] (isSignInSuccess) in
            if(isSignInSuccess){
                CCAlert.alert(title: "提示", message: "注册成功", btnTitle: "知道了", btnAction: {
                    self?.navigationController?.popViewController(animated: true)
                })
            }
        }, onError: { (error) in
            CCAlert.alert(title: "提示", message: "注册失败", btnTitle: "知道了", btnAction:nil)
        }).disposed(by: _bag)
    }
}
